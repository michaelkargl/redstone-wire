...
/**
 * Renders chains between connected RedstoneChainEntity blocks.
 * The chains sag realistically like real chains would under gravity.
 * Uses custom RenderType with position, color, and lightmap support.
 */
public class RedstoneChainRenderer implements BlockEntityRenderer<RedstoneChainEntity> {

    public static final RenderType CHAIN_RENDER_TYPE = RenderType.create(
            ...
            RenderType.CompositeState.builder()
                    ...
                    .setCullState(new RenderStateShard.CullStateShard(false)) // Disable culling to render from all angles
    ...
    public void render(RedstoneChainEntity entity, float partialTicks, PoseStack stack,
        ...
        for (BlockPos connection : entity.getConnections()) {
            ...
            // This prevents rendering the same chain twice from both ends
            if (blockPos.compareTo(connection) < 0) {
                Vec3 end = Vec3.atCenterOf(connection).subtract(Vec3.atCenterOf(blockPos));
                renderChain(stack, buffer, center.add(0.5, 0.5, 0.5), end.add(0.5, 0.5, 0.5),
    ...
    /**
     * Renders a chain between two points with realistic sagging.
     */
    private void renderChain(PoseStack poseStack, MultiBufferSource buffer,
                             Vec3 from, Vec3 to, int light, int overlay, int power) {
        VertexConsumer builder = buffer.getBuffer(CHAIN_RENDER_TYPE);

        int segments = 12;
        float thickness = 0.06F; // Increased from 0.015F for more visible chains

        PoseStack.Pose pose = poseStack.last();
        Matrix4f matrix = pose.pose();
        Matrix3f normal = pose.normal();

        // Calculate color based on redstone power
        float red = power > 0 ? 0.6f + (power / 15.0f) * 0.4f : 0.3f;
        float green = power > 0 ? 0.1f : 0.3f;
        float blue = power > 0 ? 0.1f : 0.3f;
    ...
    /**
     * Interpolates along a curved path (catenary-like curve for hanging chains).
     */
    private static Vec3 interpolateCurved(Vec3 from, Vec3 to, float t) {
        Vec3 linear = from.lerp(to, t);

        // If the chain is mostly vertical, don't add sag
        if (Math.abs(from.x - to.x) < 0.001 && Math.abs(from.z - to.z) < 0.001) {
            return linear;
        }

        // Calculate sag - chains hang down in the middle
        double curveAmplitude = 0.4;
        double curve = Math.sin(t * Math.PI) * -curveAmplitude; // Negative for downward sag

        return new Vec3(linear.x, linear.y + curve, linear.z);
    }

    /**
     * Draws a thick segment of the chain as a rectangular cuboid.
     */
    private static void drawThickSegment(VertexConsumer builder, Matrix4f matrix, Matrix3f normal,
        ...
        // Create the 8 corners of the rectangular cuboid

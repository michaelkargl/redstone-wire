...
- [x] #2 Defines `POWER` property via `createBlockStateDefinition(builder.add(POWER))`
- [x] #3 Constructor calls `registerDefaultState(stateDefinition.any().setValue(POWER, 0))`
- [x] #4 `isSignalSource()` returns `false`
- [x] #5 `getSignal()` returns `0`, `getDirectSignal()` returns `0`
- [x] #6 `hasAnalogOutputSignal()` returns `true`, `getAnalogOutputSignal()` returns POWER value
- [ ] #7 `onPlace()` schedules entity update via `level.scheduleTick(pos, this, 1)` or direct call
- [x] #8 Project compiles with `./gradlew build`
<!-- DOD:END -->

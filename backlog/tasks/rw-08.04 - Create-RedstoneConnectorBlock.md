...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Has NO POWER property — single blockstate variant
- [ ] #2 neighborChanged() is a no-op
- [ ] #3 isSignalSource() returns false, getSignal() returns 0, getDirectSignal() returns 0
- [ ] #4 hasAnalogOutputSignal() returns false
- [ ] #5 getTicker() returns null
- [ ] #6 Block does not emit or detect any redstone signal
<!-- AC:END -->
...
- [ ] #2 Has NO `POWER` property — does NOT override `createBlockStateDefinition()`
- [ ] #3 Constructor does NOT call `registerDefaultState()` with any POWER value
- [ ] #4 `neighborChanged()` is a no-op
- [ ] #5 `isSignalSource()` returns `false`
- [ ] #6 `getSignal()` returns `0`, `getDirectSignal()` returns `0`
- [ ] #7 `hasAnalogOutputSignal()` returns `false`
- [ ] #8 `getTicker()` returns `null` (inherits from base)
- [ ] #9 Block can be placed and connected via Connector item without crashes
- [ ] #10 Block has a single blockstate variant (no power variants)
